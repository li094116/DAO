...
public interface IEmpService {
    ...
    /**
     * 执行雇员数据的删除操作，可以删除多个雇员的信息，调用IEmoDAO.doRemoveBatch()方法
     ...
     * @return 查询结果以List集合的形式返回，如果没有数据则返回集合的长度为0
     * @throws Exception SQL执行异常
     */
     ...
     * 调用IEmpDAO.findAllSplit()方法，查询出所有的表的数据，返回List<Emp>
     * 调用IEmpDAO.getAllCount()方法，查询所有的数据量，返回的Integer
     * @param currentPage 当前所在页
     * @param lineSize 每页显示的记录数
     * @param column 模糊查询的数据列
     * @param keyWord 模糊查询的关键字
     * @return 本方法由于需要返回多种数据类型，所以使用Map集合返回，由于类型不统一所有value的类型设置为Object，返回内容如下：
